...
def load_checkpoint_tsne(path, model, optimizer=None):
    if os.path.isfile(path):
        logging.info("=== loading checkpoint '{}' ===".format(path))

        checkpoint = torch.load(path)
        model.load_state_dict(checkpoint['state_dict'], strict=False)        return model
...
def get_data_loader(transform_train, transform_test, config):
    ...
    test_loader = torch.utils.data.DataLoader(
        testset, batch_size=100,#config.test_batch,
